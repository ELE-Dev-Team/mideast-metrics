            ...
            try {
                const responses = await Promise.all(
                    validCountries.map(async (country) => {
                        const response = await axios.get(`http://mideast-metrics-app-env-1.eba-3yfp6yaq.us-east-1.elasticbeanstalk.com/api/v1/countries?name=${country.toLowerCase()}&year=${currentYear}`);
